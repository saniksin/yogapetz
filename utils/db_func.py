...
async def process_tasks(file_path, source_data):
    ...
    tasks_list = [
        ...
        {"type": "retweet", "tweet_id": "1752693511433093285", "status": "pending"},
    ]
    ...
    for token, data in tasks.items():
        ...
        for actual_task in data['platform']:
            if not actual_task["task"].startswith('retweet') or actual_task["task"] in [
                ...
                'retweet-yogapetz-1752693511433093285'
            ]:
                continue
        ...
        new_task = {"type": "retweet", "tweet_id": "1752693511433093285", "status": "pending"},
